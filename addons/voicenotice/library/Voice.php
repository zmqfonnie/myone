...
class Voice
{
   ...
   * */
    ...
    {
        //'updatetime'=>['gt',strtotime("-5 minutes")],
        $count = db('cms_order')->where(['status' => 'paid'])->count();
        if ($count > 0) {

            $is_save = $this->db->where(['aid' => $id])->find();

            if (empty($is_save)) {
                $data = [
                    'text' => '您有 ' . $count . ' 个订单未处理！',
                    'createtime' => time(),
                    'aid' => $id,
                    'state' => 0,
                    'loop' => 'true',
                    'url_type' => 'addtabs',
                    'url' => '/zmq.php/cms/order?ref=addtabs',
                ];
                $this->db->insert($data);
            } else {
                $this->db->where(['aid'=>$id])->setField('text','您有 ' . $count . ' 个订单未处理！');
                $this->db->where(['aid'=>$id])->setField('url','/zmq.php/cms/order?ref=addtabs');
            }
        }
