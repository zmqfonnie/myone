...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var LAYIM = function(){
    this.v = v;
    touch($('body'), '*[im-event]', function(e){
  ...
  LAYIM.prototype.setChatStatus = function(str){
    var thatChat = thisChat(), status = thatChat.elem.find('.im-chat-status');
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li im-event="chat" data-type="'+ options.type +'" data-index="'+ (options.index ? '{{'+ options.index +'}}' : (options.type === 'history' ? '{{data.type}}' : options.type) +'{{data.id}}') +'" class="im-'+ (options.type === 'history' ? '{{data.type}}' : options.type) +'{{data.id}} {{ data.status === "offline" ? "im-list-gray" : "" }}"><div><img src="{{data.avatar}}"></div><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="im-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var comTpl = function(tpl, anim, back){
    return ['<div class="im-panel'+ (anim ? ' layui-m-anim-left' : '') +'">'
      ,'<div class="im-title" style="background-color: {{d.base.chatTitleColor}};">'
        ,'<p>'
          ,(back ? '<i class="layui-icon im-chat-back" im-event="back">&#xe603;</i>' : '')
          ,'{{ d.title || d.base.title }}<span class="im-chat-status"></span>'
          ,'{{# if(d.data){ }}'
            ,'{{# if(d.data.type === "group"){ }}'
              ,'<i class="layui-icon im-chat-detail" im-event="detail">&#xe613;</i>'
            ,'{{# } }}'
          ,'{{# } }}'
        ,'</p>'
      ,'</div>'
      ,'<div class="layui-unselect im-content">'
        ,tpl
      ,'</div>'
    ,'</div>'].join('');
  };
  ...
  //主界面模版
  var elemTpl = ['<div class="layui-im">'
    ,'<div class="im-tab-content layui-show">'
      ,'<ul class="im-list-friend">'
    ...
    ,'<div class="im-tab-content">'
      ,'<ul class="im-list-top">'
        ,'{{# if(d.base.isNewFriend){ }}'
        ,'<li im-event="newFriend"><i class="layui-icon">&#xe654;</i>新的朋友<i class="im-new" id="LAY_layimNewFriend"></i></li>'
        ,'{{# } if(d.base.isgroup){ }}'
        ,'<li im-event="group"><i class="layui-icon">&#xe613;</i>群聊<i class="im-new" id="LAY_layimNewGroup"></i></li>'
        ,'{{# } }}'
      ,'</ul>'
      ,'<ul class="im-list-friend">'
    ...
    ,'<div class="im-tab-content">'
      ,'<ul class="im-list-top">'
        ...
        ,'<li im-event="moreList" lay-filter="{{ item.alias }}">'
          ,'<i class="layui-icon {{item.iconClass||\"\"}}">{{item.iconUnicode||""}}</i>{{item.title}}<i class="im-new" id="LAY_layimNew{{ item.alias }}"></i>'
        ,'</li>'
        ,'{{# }); if(!d.base.copyright){ }}'
        ,'<li im-event="about"><i class="layui-icon">&#xe60b;</i>关于<i class="im-new" id="LAY_layimNewAbout"></i></li>'
        ,'{{# } }}'
      ,'</ul>'
    ,'</div>'
  ,'</div>'
  ,'<ul class="layui-unselect layui-im-tab">'
    ,'<li title="消息" im-event="tab" lay-type="message" class="im-this"><i class="layui-icon">&#xe611;</i><span>消息</span><i class="im-new" id="LAY_layimNewMsg"></i></li>'
    ,'<li title="联系人" im-event="tab" lay-type="friend"><i class="layui-icon">&#xe612;</i><span>联系人</span><i class="im-new" id="LAY_layimNewList"></i></li>'
    ,'<li title="更多" im-event="tab" lay-type="more"><i class="layui-icon">&#xe670;</i><span>更多</span><i class="im-new" id="LAY_layimNewMore"></i></li>'
  ,'</ul>'].join('');
  
  //聊天主模板
  var elemChatTpl = ['<div class="im-chat im-chat-{{d.data.type}}">'
    ,'<div class="im-chat-main">'
      ,'<ul></ul>'
    ,'</div>'
    ,'<div class="im-chat-footer">'
      ,'<div class="im-chat-send"><input type="text" autocomplete="off"><button class="im-send layui-disabled" im-event="send">发送</button></div>'
      ,'<div class="im-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ,'<span class="layui-icon im-tool-face" title="选择表情" im-event="face">&#xe60c;</span>'
        ,'{{# if(d.base && d.base.uploadImage){ }}'
        ,'<span class="layui-icon im-tool-image" title="上传图片" im-event="image">&#xe60d;<input type="file" name="file" accept="image/*"></span>'
        ,'{{# }; }}'
        ...
        ,'<span class="layui-icon  {{item.iconClass||\"\"}} im-tool-{{item.alias}}" title="{{item.title}}" im-event="extend" lay-filter="{{ item.alias }}">{{item.iconUnicode||""}}</span>'
         ,'{{# }); }}'
      ,'</div>'
    ,'</div>'
  ,'</div>'].join('');
  ...
  var elemChatMain = ['<li class="im-chat-li{{ d.mine ? " im-chat-mine" : "" }}">'
    ,'<div class="im-chat-user"><img src="{{ d.avatar }}"><cite>'
      ,'{{ d.username||"佚名" }}'
    ,'</cite></div>'
    ,'<div class="im-chat-text">{{ layui.data.content(d.content||"&nbsp;") }}</div>'
  ,'</li>'].join('');
    ...
    return layer.open({
     type: 1
      ...
      ,success: function(elem){
        layimMain = $(elem);
        fixIosScroll(layimMain.find('.layui-im'));
        if(cache.base.tabIndex){
          events.tab($('.layui-im-tab>li').eq(cache.base.tabIndex));
        }
      }
    });
  };
  ...
  var popPanel = function(options, anim){
    ...
    return layer.open({
      ...
      ,success: function(elem){
        var othis = $(elem);
        othis.prev().find('.im-panel').addClass('layui-m-anim-lout');
        options.success && options.success(elem);
        options.isChat || fixIosScroll(othis.find('.im-content'));
      }
      ,end: options.end
    });
  }
  ...
  var thisChat = function(){
    if(!layimChat) return {};
    var cont = layimChat.find('.im-chat');
  ...
  var setHistory = function(data){
    ...
    var msgItem = historyElem.find('.im-'+ data.type + data.id)
    ,msgNums = (cache.message[data.type+data.id]||[]).length //未读消息数
    ,showMsg = function(){
      msgItem = historyElem.find('.im-'+ data.type + data.id);
      msgItem.find('p').html(data.content);
      if(msgNums > 0){
        msgItem.find('.im-msg-status').html(msgNums).addClass(SHOW);
      }
    };
  ...
  var sendMessage = function(){
    ...
    if(time - (sendMessage.time||0) > 60*1000){
      ul.append('<li class="im-chat-system"><span>'+ layui.data.date() +'</span></li>');
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if(data.system){
      ...
      if(data.timestamp - (sendMessage.time||0) > 60*1000){
        ul.append('<li class="im-chat-system"><span>'+ layui.data.date(data.timestamp) +'</span></li>');
  ...
  var viewChatlog = function(){
    ...
    var ul = thatChat.elem.find('.im-chat-main ul');
    layui.each(chatlog[thatChat.data.type + thatChat.data.id], function(index, item){
      if(new Date().getTime() > item.timestamp && item.timestamp - (sendMessage.time||0) > 60*1000){
        ul.append('<li class="im-chat-system"><span>'+ layui.data.date(item.timestamp) +'</span></li>');
        ...
        layui.each(cache.friend, function(index1, item1){
          layui.each(item1.list, function(index, item){
            if(data.id == item.id){
              ...
              var list = li.find('.layui-im-list').children('li');
              li.find('.layui-im-list').children('li').eq(index).remove();
  ...
  var chatListMore = function(){
    var thatChat = thisChat(), chatMain = thatChat.elem.find('.im-chat-main');
    var ul = chatMain.find('ul'), li = ul.children('.im-chat-li');
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    chat: function(othis){
      ...
      popchat(data, true);
      $('.im-'+ data.type + data.id).find('.im-msg-status').removeClass(SHOW);
    }
    ...
    //底部导航切换
    ,tab: function(othis){
      var index = othis.index(), main = '.im-tab-content';
    ...
    ,face: function(othis, e){
      ...
      content = '<ul class="layui-im-face">'+ content +'</ul>';
      layer.popBottom({
        content: content
        ,success: function(elem){
          var list = $(elem).find('.layui-im-face').children('li')
          touch(list, function(){
    ...
    ,group: function(){
      popPanel({
        title: '群聊'
        ,tpl: ['<div class="layui-im-list im-list-group">'
          ,listTpl({
            type: 'group'
            ,item: 'd.group'
          })
    ...
    ,chatLog: function(othis){
      ...
      layui.each(call.chatlog, function(index, item){
        item && item(thatChat.data, thatChat.elem.find('.im-chat-main>ul'));
      });
    }
    ...
    ,about: function(){
      layer.open({
        content: '<p style="padding-bottom: 5px;">LayIM属于付费产品，欢迎通过官网获得授权，促进良性发展！</p><p>当前版本：im mobile v'+ v + '</p><p>版权所有：<a href="http://im.layui.com" target="_blank">im.layui.com</a></p>'
        ,className: 'im-about'
        ,shadeClose: false
        ,btn: '我知道了'
      });
  ...
  'modules/im/mobile/im.css?v=2.10'
  ,'skinlayim-mobilecss'
);