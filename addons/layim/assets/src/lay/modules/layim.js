...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var SHOW = 'layui-show', THIS = 'im-this', MAX_ITEM = 20;
  ...
  var LAYIM = function(){
    this.v = v;
    $('body').on('click', '*[im-event]', function(e){
      var othis = $(this), methid = othis.attr('im-event');
  ...
  LAYIM.prototype.config = function(options){
    ...
    layui.each(Array(5), function(index){
      skin.push(layui.cache.dir+'css/modules/im/skin/'+ (index+1) +'.jpg')
    });
    options = options || {};
  ...
  LAYIM.prototype.setChatStatus = function(str){
    ...
    if(!thatChat) return;
    var status = thatChat.elem.find('.im-chat-status');
  ...
  LAYIM.prototype.setFriendStatus = function(id, type){
    var list = $('.im-friend'+ id);
    list[type === 'online' ? 'removeClass' : 'addClass']('im-list-gray');
  };
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li im-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" class="im-'+ (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') +' {{ data.status === "offline" ? "im-list-gray" : "" }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="im-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
      ,'<li class="im-null">'+ (nodata[options.type] || "暂无数据") +'</li>'
    ,'{{# } }}'].join('');
  };

  var elemTpl = ['<div class="layui-im-main">'
    ,'<div class="layui-im-info">'
      ,'<div class="layui-im-user">{{ d.mine.username }}</div>'
      ,'<div class="layui-im-status">'
        ,'{{# if(d.mine.status === "online"){ }}'
        ,'<span class="layui-icon im-status-online" im-event="status" lay-type="show">&#xe617;</span>'
        ,'{{# } else if(d.mine.status === "hide") { }}'
        ,'<span class="layui-icon im-status-hide" im-event="status" lay-type="show">&#xe60f;</span>'
        ,'{{# } }}'
        ,'<ul class="layui-anim im-menu-box">'
          ,'<li {{d.mine.status === "online" ? "class=im-this" : ""}} im-event="status" lay-type="online"><i class="layui-icon">&#xe605;</i><cite class="layui-icon im-status-online">&#xe617;</cite>在线</li>'
          ,'<li {{d.mine.status === "hide" ? "class=im-this" : ""}} im-event="status" lay-type="hide"><i class="layui-icon">&#xe605;</i><cite class="layui-icon im-status-hide">&#xe60f;</cite>隐身</li>'
        ,'</ul>'
      ,'</div>'
      ,'<input class="layui-im-remark" placeholder="编辑签名" value="{{ d.mine.remark||d.mine.sign||"" }}">'
    ,'</div>'
    ,'<ul class="layui-unselect layui-im-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}'
      ,' im-tab-two'
    ,'{{# } }}">'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isfriend){ }}'
      ...
      ,' im-this'
      ,'{{# } }}'
      ,'" title="联系人" im-event="tab" lay-type="friend">&#xe612;</li>'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isgroup){ }}'
      ,' im-hide'
      ...
      ,' im-this'
      ,'{{# } }}'
      ,'" title="群组" im-event="tab" lay-type="group">&#xe613;</li>'
      ,'<li class="layui-icon" title="历史会话" im-event="tab" lay-type="history">&#xe611;</li>'
    ,'</ul>'
    ,'<ul class="layui-unselect im-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} im-list-friend">'
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 im-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="im-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
        ,'<ul class="layui-im-list {{# if(spread === "true"){ }}'
        ,' layui-show'
        ,'{{# } }}">'
        ,listTpl({
          type: "friend"
    ...
    ,'{{# }); if(d.friend.length === 0){ }}'
      ,'<li><ul class="layui-im-list layui-show"><li class="im-null">暂无联系人</li></ul>'
    ,'{{# } }}'
    ,'</ul>'
    ,'<ul class="layui-unselect im-tab-content {{# if(!d.base.isfriend && d.base.isgroup){ }}layui-show{{# } }}">'
      ,'<li>'
        ,'<ul class="layui-im-list layui-show im-list-group">'
        ,listTpl({
          type: 'group'
        })
        ,'</ul>'
    ...
    ,'<ul class="layui-unselect im-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">'
      ,'<li>'
        ,'<ul class="layui-im-list layui-show im-list-history">'
        ,listTpl({
          type: 'history'
        })
        ,'</ul>'
      ,'</li>'
    ,'</ul>'
    ,'<ul class="layui-unselect im-tab-content">'
      ,'<li>'
        ,'<ul class="layui-im-list layui-show" id="layui-im-search"></ul>'
      ,'</li>'
    ,'</ul>'
    ,'<ul class="layui-unselect layui-im-tool">'
      ,'<li class="layui-icon im-tool-search" im-event="search" title="搜索">&#xe615;</li>'
      ,'{{# if(d.base.msgbox){ }}'
      ,'<li class="layui-icon im-tool-msgbox" im-event="msgbox" title="消息盒子">&#xe645;<span class="layui-anim"></span></li>'
      ,'{{# } }}'
      ,'{{# if(d.base.find){ }}'
      ,'<li class="layui-icon im-tool-find" im-event="find" title="查找">&#xe608;</li>'
      ,'{{# } }}'
      ,'<li class="layui-icon im-tool-skin" im-event="skin" title="更换背景">&#xe61b;</li>'
      ,'{{# if(!d.base.copyright){ }}'
      ,'<li class="layui-icon im-tool-about" im-event="about" title="关于">&#xe60b;</li>'
      ,'{{# } }}'
    ,'</ul>'
    ,'<div class="layui-im-search"><input><label class="layui-icon" im-event="closeSearch">&#x1007;</label></div>'
  ,'</div>'].join('');
  
  //换肤模版
  var elemSkinTpl = ['<ul class="layui-im-skin">'
  ,'{{# layui.each(d.skin, function(index, item){ }}'
    ,'<li><img im-event="setSkin" src="{{ item }}"></li>'
  ,'{{# }); }}'
  ,'<li im-event="setSkin"><cite>简约</cite></li>'
  ,'</ul>'].join('');
  
  //聊天主模板
  var elemChatTpl = ['<div class="im-chat im-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect im-chat-title">'
      ,'<div class="im-chat-other">'
        ,'<img class="im-{{ d.data.type }}{{ d.data.id }}" src="{{ d.data.avatar }}"><span class="im-chat-username" im-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"佚名" }} {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="im-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
        ,'<p class="im-chat-status"></p>'
      ,'</div>'
    ,'</div>'
    ...
    ,'<div class="im-chat-footer">'
      ,'<div class="layui-unselect im-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ,'<span class="layui-icon im-tool-face" title="选择表情" im-event="face">&#xe60c;</span>'
        ,'{{# if(d.base && d.base.uploadImage){ }}'
        ,'<span class="layui-icon im-tool-image" title="上传图片" im-event="image">&#xe60d;<input type="file" name="file"></span>'
        ,'{{# }; }}'
        ,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon im-tool-image" title="发送文件" im-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>'
         ,'{{# }; }}'
         ,'{{# if(d.base && d.base.isAudio){ }}'
        ,'<span class="layui-icon im-tool-audio" title="发送网络音频" im-event="media" data-type="audio">&#xe6fc;</span>'
         ,'{{# }; }}'
         ,'{{# if(d.base && d.base.isVideo){ }}'
        ,'<span class="layui-icon im-tool-video" title="发送网络视频" im-event="media" data-type="video">&#xe6ed;</span>'
         ,'{{# }; }}'
         ,'{{# layui.each(d.base.tool, function(index, item){ }}'
        ,'<span class="layui-icon im-tool-{{item.alias}}" title="{{item.title}}" im-event="extend" lay-filter="{{ item.alias }}">{{item.icon}}</span>'
         ,'{{# }); }}'
        ,'{{# if(d.base && d.base.chatLog){ }}'
        ,'<span class="im-tool-log" im-event="chatLog"><i class="layui-icon">&#xe60e;</i>聊天记录</span>'
        ,'{{# }; }}'
      ,'</div>'
      ,'<div class="im-chat-textarea"><textarea></textarea></div>'
      ,'<div class="im-chat-bottom">'
        ,'<div class="im-chat-send">'
          ,'{{# if(!d.base.brief){ }}'
          ,'<span class="im-send-close" im-event="closeThisChat">关闭</span>'
          ,'{{# } }}'
          ,'<span class="im-send-btn" im-event="send">发送</span>'
          ,'<span class="im-send-set" im-event="setSend" lay-type="show"><em class="layui-edge"></em></span>'
          ,'<ul class="layui-anim im-menu-box">'
            ,'<li {{d.local.sendHotKey !== "Ctrl+Enter" ? "class=im-this" : ""}} im-event="setSend" lay-type="Enter"><i class="layui-icon">&#xe618;</i>按Enter键发送消息</li>'
            ,'<li {{d.local.sendHotKey === "Ctrl+Enter" ? "class=im-this" : ""}} im-event="setSend"  lay-type="Ctrl+Enter"><i class="layui-icon">&#xe618;</i>按Ctrl+Enter键发送消息</li>'
          ,'</ul>'
        ,'</div>'
      ,'</div>'
    ,'</div>'
  ...
  //添加好友群组模版
  var elemAddTpl = ['<div class="im-add-box">'
    ,'<div class="im-add-img"><img class="layui-circle" src="{{ d.data.avatar }}"><p>{{ d.data.name||"" }}</p></div>'
    ,'<div class="im-add-remark">'
  ...
  //聊天内容列表模版
  var elemChatMain = ['<li {{ d.mine ? "class=im-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
    ,'<div class="im-chat-user"><img src="{{ d.avatar }}"><cite>'
    ,'{{# if(d.mine){ }}'
     ...
     ,'{{# } }}'
      ,'</cite></div>'
    ,'<div class="im-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>'
  ,'</li>'].join('');
  
  var elemChatList = '<li class="im-{{ d.data.type }}{{ d.data.id }} im-chatlist-{{ d.data.type }}{{ d.data.id }} im-this" im-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" im-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  layui.data.content = function(content){
    ...
    .replace(/img\[([^\s]+?)\]/g, function(img){  //转义图片
      return '<img class="layui-im-photos" src="' + img.replace(/(^img\[)|(\]$)/g, '') + '">';
    })
    .replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g, function(str){ //转义文件
      var href = (str.match(/file\(([\s\S]+?)\)\[/)||[])[1];
      var text = (str.match(/\)\[([\s\S]*?)\]/)||[])[1];
      if(!href) return str;
      return '<a class="layui-im-file" href="'+ href +'" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>'+ (text||href) +'</cite></a>';
    })
    .replace(/audio\[([^\s]+?)\]/g, function(audio){  //转义音频
      return '<div class="layui-unselect layui-im-audio" im-event="playAudio" data-src="' + audio.replace(/(^audio\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>';
    })
    .replace(/video\[([^\s]+?)\]/g, function(video){  //转义音频
      return '<div class="layui-unselect layui-im-video" im-event="playVideo" data-src="' + video.replace(/(^video\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe652;</i></div>';
    })
  ...
  var layimMain, popim = function(content){
    return layer.open({
      ...
      ,success: function(layero){
        ...
        //按最新会话重新排列
        var arr = [], historyElem = layero.find('.im-list-history');
  ...
  var banRightMenu = function(){
    ...
    //自定义历史会话右键菜单
    layimMain.find('.im-list-history').on('contextmenu', 'li', function(e){
      var othis = $(this);
      var html = '<ul data-id="'+ othis[0].id +'" data-index="'+ othis.data('index') +'"><li im-event="menuHistory" data-type="one">移除该会话</li><li im-event="menuHistory" data-type="all">清空全部会话列表</li></ul>';
      
      if(othis.hasClass('im-null')) return;
      ...
      layer.tips(html, this, {
        ...
        ,anim: 5
        ,fixed: true
        ,skin: 'layui-box layui-im-contextmenu'
        ,success: function(layero){
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,title: false
      ,id: 'layui-im-close'
      ,skin: 'layui-box layui-im-min layui-im-close'
      ,shade: false
      ,closeBtn: false
      ,anim: 2
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/im/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title||'我的LayIM') +'</span>'
      ,move: '#layui-im-close img'
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(chat[0]){
      var list = layimChat.find('.im-chat-list');
      var listThat = list.find('.im-chatlist-'+ data.type + data.id);
      var hasFull = layimChat.find('.layui-layer-max').hasClass('layui-layer-maxmin');
      var chatBox = chat.children('.im-chat-box');
      ...
      changeChat(list.find('.im-chatlist-'+ data.type + data.id));
    ...
    var index = chatIndex = layer.open({
      ...
      ,content: laytpl('<ul class="layui-unselect im-chat-list">'+ elemChatList +'</ul><div class="im-chat-box">' + elemChatTpl + '</div>').render(render)
      ,success: function(layero){
        layimChat = layero;
        ...
        //查看大图
        layero.on('dblclick', '.layui-im-photos', function(){
          var src = this.src;
  ...
  var syncGray = function(data){
    $('.im-'+data.type+data.id).each(function(){
  ...
  var resizeChat = function(){
    var list = layimChat.find('.im-chat-list')
    ,chatMain = layimChat.find('.im-chat-main')
  ...
  var setChatMin = function(newMsg){
    ...
    setChatMin.index = layer.open({
      ...
      ,resize: false
      ,area: ['182px', '50px']
      ,content: '<img id="layui-im-min" src="'+ thatChat.avatar +'"><span>'+ thatChat.name +'</span>'
      ,success: function(layero, index){
  ...
  var changeChat = function(elem, del){
    elem = elem || $('.im-chat-list .' + THIS);
    var index = elem.index() === -1 ? 0 : elem.index();
    var str = '.im-chat', cont = layimChat.find(str).eq(index);
    var hasFull = layimChat.find('.layui-layer-max').hasClass('layui-layer-maxmin');
    ...
    if(del){
      ...
      //只剩下1个列表，隐藏左侧区块
      if(length === 2){
        layimChat.find('.im-chat-list').hide();
        if(!hasFull){
          layimChat.css('width', '600px');
        }
        layimChat.find('.im-chat-box').css('margin-left', 0);
      } 
  ...
  var thisChat = LAYIM.prototype.thisChat = function(){
    if(!layimChat) return;
    var index = $('.im-chat-list .' + THIS).index();
    var cont = layimChat.find('.im-chat').eq(index);
    var to = JSON.parse(decodeURIComponent(cont.find('.im-chat-tool').data('json')));
    return {
      elem: cont
      ,data: to
    ...
    layero.css({
      'background-image': skin ? 'url('+ skin +')' : function(){
        return cache.base.initSkin 
          ? 'url('+ (layui.cache.dir+'css/modules/im/skin/'+ cache.base.initSkin) +')'
        : 'none';
      }()
    });
  };
  ...
  var setHistory = function(data){
    ...
    var historyElem = layimMain.find('.im-list-history');
    ...
    historyElem.find('.im-null').remove();
  };
  ...
  var sendMessage = function(){
    var data = {
      ...
      ,avatar: cache.mine ? cache.mine.avatar : (layui.cache.dir+'css/pc/im/skin/logo.jpg')
      ,id: cache.mine ? cache.mine.id : null
      ,mine: true
    };
    var thatChat = thisChat(), ul = thatChat.elem.find('.im-chat-main ul');
  ...
  var voice = function() {
    ...
    audio.src = layui.cache.dir+'css/modules/im/voice/'+ cache.base.voice;
    audio.play();
  };
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    var elem = $('.im-chatlist-'+ data.type + data.id);
    ...
    var cont = layimChat.find('.im-chat').eq(index);
    var ul = cont.find('.im-chat-main ul');
    
    //系统消息
    if(data.system){
      if(index !== -1){
        ul.append('<li class="im-chat-system"><span>'+ data.content +'</span></li>');
      }
  ...
  var ANIM_MSG = 'layui-anim-loop layer-anim-05', msgbox = function(num){
    var msgboxElem = layimMain.find('.im-tool-msgbox');
  ...
  var viewChatlog = function(){
    ...
    ,ul = thatChat.elem.find('.im-chat-main ul');
  ...
  var addList = function(data){
    var obj = {}, has, listElem = layimMain.find('.im-list-'+ data.type);
    ...
    if(data.type === 'friend'){
      ...
      li.find('.layui-im-list').append(list);
      li.find('.im-count').html(cache.friend[data.groupIndex].list.length); //刷新好友数量
      //如果初始没有好友
      if(li.find('.im-null')[0]){
        li.find('.im-null').remove();
      }
    } else if(data.type === 'group'){
      listElem.append(list);
      //如果初始没有群组
      if(listElem.find('.im-null')[0]){
        listElem.find('.im-null').remove();
      }
    }
  };
  ...
  var removeList = function(data){
    var listElem = layimMain.find('.im-list-'+ data.type);
    var obj = {};
    if(cache[data.type]){
      if(data.type === 'friend'){
        layui.each(cache.friend, function(index1, item1){
          layui.each(item1.list, function(index, item){
            if(data.id == item.id){
              ...
              var list = li.find('.layui-im-list>li');
              li.find('.layui-im-list>li').eq(index).remove();
              cache.friend[index1].list.splice(index, 1); //从cache的friend里面也删除掉好友
              li.find('.im-count').html(cache.friend[index1].list.length); //刷新好友数量
              //如果一个好友都没了
              if(cache.friend[index1].list.length === 0){
                li.find('.layui-im-list').html('<li class="im-null">该分组下已无好友了</li>');
              }
              return true;
            }
          });
        ...
        layui.each(cache.group, function(index, item){
          if(data.id == item.id){
            ...
            if(cache.group.length === 0){
              listElem.html('<li class="im-null">暂无群组</li>');
            }
            return true;
          }
        });
  ...
  var chatListMore = function(){
    var thatChat = thisChat(), chatMain = thatChat.elem.find('.im-chat-main');
    var ul = chatMain.find('ul');
    var length = ul.find('li').length;
    
    if(length >= MAX_ITEM){
      var first = ul.find('li').eq(0);
      if(!ul.prev().hasClass('im-chat-system')){
        ul.before('<div class="im-chat-system"><span im-event="chatLog">查看更多记录</span></div>');
      }
      if(length > MAX_ITEM){
        first.remove();
      }
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    status: function(othis, e){
      ...
      if(type === 'show'){
        ...
        prev.removeClass('im-status-'+(type === 'online' ? 'hide' : 'online'))
        .addClass('im-status-'+type);
    ...
    //编辑签名
    ,sign: function(){
      var input = layimMain.find('.layui-im-remark');
    ...
    //大分组切换
    ,tab: function(othis){
      var index, main = '.im-tab-content';
      var tabs = layimMain.find('.layui-im-tab>li');
      typeof othis === 'number' ? (
        index = othis
        ,othis = tabs.eq(index)
      ) : (
    ...
    ,search: function(othis){
      var search = layimMain.find('.layui-im-search');
      var main = layimMain.find('#layui-im-search');
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li im-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
            html = '<li class="im-null">无搜索结果</li>';
          }
          main.html(html);
          events.tab(3);
        }
    ...
    //关于
    ,about: function(){
      layer.alert('版本： '+ v + '<br>版权所有：<a href="http://im.layui.com" target="_blank">im.layui.com</a>', {
        title: '关于 LayIM'
        ,shade: false
      });
    }
    ...
    ,setSkin: function(othis){
      ...
      layui.each(call.setSkin, function(index, item){
        var filename = (src||'').replace(layui.cache.dir+'css/modules/im/skin/', '');
    ...
    ,groupMembers: function(othis, e){
      ...
      if(othis.data('down')){
        ...
        events.groupMembers.index = layer.tips('<ul class="im-members-list"></ul>', othis, {
          tips: 3
          ,time: 0
          ,anim: 5
          ,fixed: true
          ,skin: 'layui-box layui-im-members'
          ,success: function(layero){
            var members = cache.base.members || {}, thatChat = thisChat()
            ,ul = layero.find('.im-members-list'), li = '', membersCache = {}
            ,hasFull = layimChat.find('.layui-layer-max').hasClass('layui-layer-maxmin')
            ,listNone = layimChat.find('.im-chat-list').css('display') === 'none';
            if(hasFull){
              ul.css({
            ...
            post(members, function(res){
              ...
              //获取群员
              othis.find('.im-chat-members').html(res.members||(res.list||[]).length + '人');
    ...
    ,setSend: function(othis, e){
      var box = events.setSend.box = othis.siblings('.im-menu-box')
    ...
    ,face: function(othis, e){
      ...
      content = '<ul class="layui-clear im-face-list">'+ content +'</ul>';
     
     events.face.index = layer.tips(content, othis, {
        tips: 1
        ,time: 0
        ,fixed: true
        ,skin: 'layui-box layui-im-face'
        ,success: function(layero){
          layero.find('.im-face-list>li').on('mousedown', function(e){
            stope(e);
          }).on('click', function(){
    ...
    ,menuHistory: function(othis, e){
      ...
      var hisElem = layimMain.find('.im-list-history');
      var none = '<li class="im-null">暂无历史会话</li>'
